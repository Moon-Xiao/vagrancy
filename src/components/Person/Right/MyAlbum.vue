      ...
      <template slot="list-item" scope="item">
        ...
        <span @click='remove($event, item.value._id)' class="remove-style">
          ...
          <router-link :to="`/show-album/${item.value._id}`">
            <div class="row-item">
              <div class="item-img" :style="`background-image: url('${baseUrl}/${item.value.avatar.path}')`"></div>
...
<script>
  ...
  export default {
    components: {PaginateList, Paging},
    data () {
      return {
        showEdit: false,
    ...
    methods: {
      ...
      isShowCreateAlbum: function () {
        this.$refs.createAlbum.show()
      },
      isShowEdit: function () {
...
<style>
  ...
  #album .row .row-item .item-img:hover {
  ...
  #album.photo-top .top-btns {
    width: 38%;
    float: right
  }

  #album.photo-top .top-btn {
    height: 2.5rem;
    margin-right: 1rem;
    padding: 0.5rem;
    color: white;
  ...
  #album.photo-nav .remove-style > i {
    font-size: 0.83rem;
  }

  #album.photo-nav .row-item > div {
