<template>
  <div id="per">
    ...
    <div class="per-nav">
      ...
      <div class="rate">
        <span>等级：</span>
        <span style="color: red">{{info.level}}</span>
      </div>
      <!--signature-->
      ...
      <div class="bottom-bar row">
        ...
        <div class="bottom-item col-md-4">
          <div>{{info.follow ? info.follow.length : 0}}</div>
        ...
        <div class="bottom-item col-md-4">
          <div>{{info.like ? info.like.length : 0}}</div>
...
<script>
  ...
  export default {
    ...
    computed: {
      tags () {
        return (this.info.tags || '').split(' ').map(s => s.trim()).filter(s => s !== '')
      },
      isSignShow () {
    ...
    async mounted () {
      this.signature = this.info.intro
